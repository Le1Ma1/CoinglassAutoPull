...
def _merge_lsr(lsr_g, lsr_a, lsr_p):
    frames = []
    ...
    if lsr_a is not None and len(lsr_a):
        x = lsr_a.rename(columns={"long_short_ratio":"lsr_top_accts"})
        frames.append(x[["symbol","ts_utc","date_utc","lsr_top_accts"]])
    if lsr_p is not None and len(lsr_p):
        x = lsr_p.rename(columns={"long_short_ratio":"lsr_top_pos"})
...
def _dedupe_cols(df: pd.DataFrame) -> pd.DataFrame:
    """保留第一個同名欄位，刪除後續重複，避免 df['ts_utc'] 變成 DataFrame。"""
    dup_mask = df.columns.duplicated(keep="first")
    if dup_mask.any():
        dup_cols = pd.Index(df.columns[dup_mask]).unique().tolist()
        print(f"[fix] 發現重複欄位: {dup_cols} -> 保留第一個，刪除其餘")
...
def _build_labels(df_all: pd.DataFrame, start_date: date, end_date: date) -> pd.DataFrame:
    ...
    return df.loc[(df["date_utc"]>=start_date)&(df["date_utc"]<=end_date), ["asset","ts_utc","date_utc","fwd_1d_ret"]].reset_index(drop=True)

def _upload_features(feat: pd.DataFrame):
    feat = _dedupe_cols(feat)
    with get_conn() as conn:
        copy_upsert_chunks(conn, "public", "features_1d", feat, pk=("asset","ts_utc"), chunk_rows=200_000, log=print)
...
def main():
    ...
    alias = {
        "spot":"來源 spot","fut":"來源 fut","oi":"來源 oi","oi_stable":"來源 oi_stable","oi_coinm":"來源 oi_coinm",
        "funding_oiw":"來源 funding_oiw","funding_volw":"來源 funding_volw","lsr_g":"來源 lsr_g","lsr_a":"來源 lsr_a","lsr_p":"來源 lsr_p",
        "ob":"來源 ob","taker":"來源 taker","liq":"來源 liq","etf_flow":"來源 etf_flow","etf_aum":"來源 etf_aum","etf_prem":"來源 etf_prem",
        "etf_hk":"來源 etf_hk","cpi":"來源 cpi","bfx":"來源 bfx","bir":"來源 bir","puell":"來源 puell","s2f":"來源 s2f","pi":"來源 pi",
    }
