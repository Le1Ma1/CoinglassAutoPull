...
def copy_upsert_chunks(df: pd.DataFrame, table="public.features_1d", chunk_rows=100_000, prefix=""):
    ...
    if "s2f_next_halving" in df.columns:
        x = pd.to_numeric(df["s2f_next_halving"], errors="coerce")  # 可能是 990.0
        df["s2f_next_halving"] = x.round().astype("Int64")          # 變成 990 或 <NA>
    ...
    with connect() as conn, conn.cursor() as cur:
        cur.execute("begin; create temp table _stage_features_1d (like public.features_1d including all);")
        n = len(df); chunks = max(1, math.ceil(n / chunk_rows))
        for i in range(chunks):
            g = df.iloc[i*chunk_rows:(i+1)*chunk_rows]
            buf = io.StringIO()
            g.to_csv(buf, index=False, na_rep="")  # 空字串代表 NULL
            buf.seek(0)
            cur.copy_expert(
        ...
        cur.execute(f"""
            ...
            on conflict (asset, ts_utc) do update set {set_clause}, updated_at=now();
        """)
        cur.execute("commit;")